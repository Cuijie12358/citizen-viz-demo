...
### 3. 📝 提交我的数据（新增！）
- **用户数据提交** — 点击右下角 📝 按钮，提交自己的申请记录
- **本地隐私存储** — 数据仅存储在浏览器 localStorage，不上传任何服务器
- **数据验证** — 字段范围校验 + 频率限制（60 秒一次），防止误填
- **图表融合** — 提交的数据自动与历史记录合并，参与所有图表分析
- **高亮显示** — 自己的数据在表格中以蓝色"我的"标记高亮
- **导出功能** — 一键导出所有已提交记录为 JSON 文件
...
# 强制重新爬取（忽略 7 天缓存）
python3 context/scraper.py --force-refresh
```

> **缓存机制**：爬虫默认缓存原始数据 7 天（`context/cache/`），避免频繁请求。
> 修改特征提取逻辑时需更新 `scraper.py` 中的 `EXTRACTOR_VERSION` 变量。
