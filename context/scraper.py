    ...
    options: {
      ...
      plugins: {
        legend: {
          position:'right',
          labels: {
            color:'#666', boxWidth:12, font:{size:11},
            generateLabels: chart => {
              const tot = chart.data.datasets[0].data.reduce((a,b)=>a+b,0);
              return chart.data.labels.map((lbl,i) => {
                const v = chart.data.datasets[0].data[i];
                return { text:`${lbl}: ${v} (${tot?(v/tot*100).toFixed(1):0}%)`, fillStyle:chart.data.datasets[0].backgroundColor[i], strokeStyle:chart.data.datasets[0].borderColor[i], lineWidth:2, fontColor:'#666', index:i };
              });
            }
          }
        },
